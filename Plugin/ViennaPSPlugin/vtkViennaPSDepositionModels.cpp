...
void registerSelectiveEpitaxyProcessModel() {
    ...
    epitaxyProcess.description = "Selective epitaxial growth model with crystal orientation-dependent rates";
    ...
    epitaxyMaterialsParam.documentation = "Select materials where epitaxial growth is allowed";
    ...
    materialRateParam.documentation = "Rate multiplier for selected epitaxy materials";
    ...
    rate111Param.documentation = "Epitaxial growth rate for (111) crystal orientation";
    ...
    rate100Param.documentation = "Epitaxial growth rate for (100) crystal orientation";
    ...
    growthMaterialParam.documentation = "Material to grow epitaxially (e.g., Si, SiGe, Ge)";
                      ...
                      const ParameterMap& params)
    {
        ...
        if (params.find("GrowthMaterial") != params.end()) {
            int matId = registry.getParameter<int>(params, "GrowthMaterial",
                                                   static_cast<int>(viennaps::Material::Si));
        ...
        ViennaPSModels::withDomain(psDomainVoid, dimension, [&](auto psDomain, auto dimTag) {
            constexpr int Dim = decltype(dimTag)::value;
                ...
                if (psDomain->getLevelSets().size() < 2) {
                    VPSLOG_WARNING(nullptr, "Warning: Selective epitaxy requires at least 2 level sets.");
                ...
                if (!topIsEpitaxyMaterial) {
                    VPSLOG_WARNING(nullptr, "Warning: Top material is not an epitaxy material.");
                    VPSLOG_DEBUG(nullptr, "Adding ", viennaps::MaterialMap::toString(growthMaterial), " as epitaxy layer...");

                    bool found = false;
                    for (auto& mat : materialRates) {
                        if (viennaps::MaterialMap::isMaterial(growthMaterial, mat.first)) {
                            found = true;
                            break;
                        }
                    }
                ...
                auto model = viennaps::SmartPointer<viennaps::SelectiveEpitaxy<NumericType, Dim>>::New(
                ...
                VPSLOG_DEBUG(nullptr, "Growth material: ", viennaps::MaterialMap::toString(growthMaterial));
...
void registerSingleParticleALDProcessModel() {
    ...
    aldProcess.description = "Atomic Layer Deposition model with single particle species and surface coverage dynamics";
    ...
    numCyclesParam.documentation = "Number of ALD cycles to simulate in one advection step";
    ...
    coverageTimeStepParam.documentation = "Time step for solving surface coverage equation";
    ...
    gasMFPParam.documentation = "Mean free path of particles in gas phase. 0 = ballistic transport";
                      ...
                      const ParameterMap& params)
    {
        ...
        double growthPerCycle = registry.getParameter<double>(params, "GrowthPerCycle", 0.1);
        int totalCycles = registry.getParameter<int>(params, "TotalCycles", 100);
        double coverageTimeStep = registry.getParameter<double>(params, "CoverageTimeStep", 0.01);
        double evaporationFlux = registry.getParameter<double>(params, "EvaporationFlux", 0.0);
        double incomingFlux = registry.getParameter<double>(params, "IncomingFlux", 1.0);
        double saturationCoverage = registry.getParameter<double>(params, "SaturationCoverage", 1.0);
        double gasMeanFreePath = registry.getParameter<double>(params, "GasMeanFreePath", 0.0);
        ...
        ViennaPSModels::withDomain(psDomainVoid, dimension, [&](auto psDomain, auto dimTag) {
            constexpr int Dim = decltype(dimTag)::value;
                ...
                auto model = viennaps::SmartPointer<viennaps::SingleParticleALD<NumericType, Dim>>::New(
                ...
                VPSLOG_DEBUG(nullptr, "Growth per cycle: ", growthPerCycle, " nm/cycle");
                ...
                if (gasMeanFreePath > 0) {
                    VPSLOG_DEBUG(nullptr, "Gas mean free path: ", gasMeanFreePath, " nm");
                } else {
                    VPSLOG_DEBUG(nullptr, "Transport mode: Ballistic");
                }
                VPSLOG_DEBUG(nullptr, "Deposition material: ", viennaps::MaterialMap::toString(depositionMaterial));
                ...
                process.setProcessDuration(1.0); // Unit time, actual growth controlled by cycles
...
void registerTEOSPECVDProcessModel() {
    ...
    teosPECVDProcess.description = "Plasma-enhanced chemical vapor deposition of TEOS with radical and ion contributions";
    ...
    radicalStickingParam.documentation = "Sticking probability for radical species";
                      ...
                      const ParameterMap& params)
    {
        ...
        double processTime = registry.getParameter<double>(params, "ProcessTime", 10.0);
        double radicalSticking = registry.getParameter<double>(params, "RadicalSticking", 1.0);
        double radicalRate = registry.getParameter<double>(params, "RadicalRate", 1.0);
        ...
        double ionSticking = registry.getParameter<double>(params, "IonSticking", 1.0);
        double radicalOrder = registry.getParameter<double>(params, "RadicalReactionOrder", 1.0);
        double ionOrder = registry.getParameter<double>(params, "IonReactionOrder", 1.0);
        double ionMinAngle = registry.getParameter<double>(params, "IonMinAngle", 0.0);
        ...
        ViennaPSModels::withDomain(psDomainVoid, dimension, [&](auto psDomain, auto dimTag) {
            constexpr int Dim = decltype(dimTag)::value;
                ...
                auto model = viennaps::SmartPointer<viennaps::TEOSPECVD<NumericType, Dim>>::New(
                    radicalSticking, radicalRate, ionRate, ionExponent,
                    ionSticking, radicalOrder, ionOrder, ionMinAngle
                );

                VPSLOG_DEBUG(nullptr, "Radicals - Sticking: ", radicalSticking, ", Rate: ", radicalRate, " nm/s", ", Order: ", radicalOrder);
                VPSLOG_DEBUG(nullptr, "Ions - Sticking: ", ionSticking, ", Rate: ", ionRate, " nm/s", ", Order: ", ionOrder, ", Exponent: ", ionExponent);
                VPSLOG_DEBUG(nullptr, "Ion minimum reflection angle: ", ionMinAngle, " rad");
                VPSLOG_DEBUG(nullptr, "Deposition material: ", viennaps::MaterialMap::toString(depositionMaterial));
                VPSLOG_DEBUG(nullptr, "Process time: ", processTime, " s");
...
void registerTEOSDepositionProcessModel() {
    ...
    teosProcess.description = "Tetraethyl orthosilicate (TEOS) deposition model for SiO2 film growth";
    ...
    stickingP1Param.documentation = "Sticking probability for primary particle species";
    ...
    stickingP2Param.documentation = "Sticking probability for secondary particle species (dual particle model only)";
    ...
    rateP2Param.documentation = "Deposition rate for secondary particle (dual particle model only)";
    ...
    orderP2Param.documentation = "Reaction order for secondary particle (dual particle model only)";
                      ...
                      const ParameterMap& params)
    {
        ...
        double stickingP1 = registry.getParameter<double>(params, "StickingProbabilityP1", 1.0);
        double rateP1 = registry.getParameter<double>(params, "RateP1", 1.0);
        double orderP1 = registry.getParameter<double>(params, "ReactionOrderP1", 1.0);
        double stickingP2 = registry.getParameter<double>(params, "StickingProbabilityP2", 0.0);
        double rateP2 = registry.getParameter<double>(params, "RateP2", 0.0);
        double orderP2 = registry.getParameter<double>(params, "ReactionOrderP2", 0.0);
        ...
        ViennaPSModels::withDomain(psDomainVoid, dimension, [&](auto psDomain, auto dimTag) {
            constexpr int Dim = decltype(dimTag)::value;
                ...
                if (modelType == 0 || rateP2 == 0.0) {
                    model = viennaps::SmartPointer<viennaps::TEOSDeposition<NumericType, Dim>>::New(
                        stickingP1, rateP1, orderP1
                    );
                    ...
                    model = viennaps::SmartPointer<viennaps::TEOSDeposition<NumericType, Dim>>::New(
                        stickingP1, rateP1, orderP1, stickingP2, rateP2, orderP2
                    );

                    VPSLOG_DEBUG(nullptr, "Particle 1 - Sticking: ", stickingP1, ", Rate: ", rateP1, " nm/s", ", Order: ", orderP1);
                    VPSLOG_DEBUG(nullptr, "Particle 2 - Sticking: ", stickingP2, ", Rate: ", rateP2, " nm/s", ", Order: ", orderP2);
                }
