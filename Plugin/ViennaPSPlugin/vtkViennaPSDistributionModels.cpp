...
void registerSphereDistributionModel() {
    ...
    sphereDistribution.description = "Deposits or etches a spherical shape on the surface";
    ...
    radiusParam.documentation = "Radius of the sphere to be deposited/etched (nm)";
    ...
    gridDeltaParam.documentation = "Grid spacing for the level set representation (nm)";
    ...
    maskParam.documentation = "Select materials where the distribution should not be applied";
    ...
    auto sphereDistModel = [](std::shared_ptr<void> psDomainVoid,
                      int dimension,
        ...
        double processTime = registry.getParameter<double>(params,"ProcessTime", 1.0);
        ...
        VPSLOG_INFO(nullptr, "sphereDistModel: ",ViennaPSMeta::ParameterMapToString(params));

        ViennaPSModels::withDomain(psDomainVoid, dimension, [&](auto psDomain, auto dimTag) {
            constexpr int Dim = decltype(dimTag)::value;

            auto model = viennaps::SmartPointer<viennaps::SphereDistribution<NumericType, Dim>>::New(
                radius
            );
...
void registerBoxDistributionModel() {
    ...
    boxDistribution.description = "Deposits or etches a box/rectangular shape on the surface";
    ...
    auto boxDistModel = [](std::shared_ptr<void> psDomainVoid,
                      int dimension,
                      vtkDataObject* output,
                      const ParameterMap& params)  {
        auto& registry = vtkViennaPSModelRegistry::getInstance();
        ...
        VPSLOG_INFO(nullptr, "boxDistModel: ",ViennaPSMeta::ParameterMapToString(params));
        ...
        ViennaPSModels::withDomain(psDomainVoid, dimension, [&](auto psDomain, auto dimTag) {
            ...
            // Create half-axes array (BoxDistribution expects 3D array even for 2D)
            ...
            auto model = viennaps::SmartPointer<viennaps::BoxDistribution<NumericType, Dim>>::New(
                halfAxes
            );
            for (const auto &mat : maskMaterials) {
                model->addMaskMaterial(mat);
...
void registerMultiParticleProcessModel() {
    ...
    multiParticleProcess.description = "Flexible process model supporting multiple particle types with customizable interactions";
    ...
    for (int i = 1; i <= 5; i++) {
        ...
        stickingParam.documentation = "Sticking probability for neutral particle " + std::to_string(i);
        ...
        matStickingSiParam.displayName = "Neutral " + std::to_string(i) + " Si Sticking";
        matStickingSiParam.documentation = "Sticking on Si for neutral particle " + std::to_string(i) + " (-1 = use default)";
    ...
    for (int i = 1; i <= 5; i++) {
        ...
        sourcePowerParam.documentation = "Angular distribution exponent for ion " + std::to_string(i);
        ...
        meanEnergyParam.documentation = "Mean energy for ion " + std::to_string(i) + " (eV)";
        ...
        sigmaEnergyParam.documentation = "Energy standard deviation for ion " + std::to_string(i) + " (eV)";
        ...
        thresholdEnergyParam.displayName = "Ion " + std::to_string(i) + " Threshold";
        thresholdEnergyParam.documentation = "Threshold energy for ion " + std::to_string(i) + " (eV)";
        ...
        bSpParam.documentation = "Angular dependency factor for ion " + std::to_string(i) + " (-1 = disabled)";
    ...
    overallRateParam.documentation = "Overall rate multiplier (nm/s for deposition, -nm/s for etching)";
                      ...
                      const ParameterMap& params)
    {
        ...
        int numNeutrals = registry.getParameter<int>(params, "NumNeutralParticles", 1);
        int numIons = registry.getParameter<int>(params, "NumIonParticles", 1);
        int rateFunctionType = registry.getParameter<int>(params, "RateFunction", 0);
        int processType = registry.getParameter<int>(params, "ProcessType", 0);
        double overallRate = registry.getParameter<double>(params, "OverallRate", 1.0);

        VPSLOG_INFO(nullptr, "MultiParticleProcessModel: ",ViennaPSMeta::ParameterMapToString(params));
        ...
        auto model = viennaps::SmartPointer<viennaps::MultiParticleProcess<NumericType, Dim>>::New();

        for (int i = 1; i <= numNeutrals; i++) {
            std::string prefix = "Neutral" + std::to_string(i) + "_";
            double sticking = registry.getParameter<double>(params, prefix + "Sticking", 1.0);
            double stickingSi = registry.getParameter<double>(params, prefix + "StickingSi", -1.0);

            if (stickingSi >= 0) {
                std::unordered_map<viennaps::Material, NumericType> materialSticking;
                materialSticking[viennaps::Material::Si] = stickingSi;
                model->addNeutralParticle(materialSticking, sticking, "neutralFlux");
            } else {
                model->addNeutralParticle(sticking, "neutralFlux");
            }

            VPSLOG_INFO(nullptr, "Added neutral particle ", i," with sticking = ", sticking);
        }

        for (int i = 1; i <= numIons; i++) {
            std::string prefix = "Ion" + std::to_string(i) + "_";
            double sourcePower = registry.getParameter<double>(params, prefix + "SourcePower", 100.0);
            double meanEnergy = registry.getParameter<double>(params, prefix + "MeanEnergy", 100.0);
            double sigmaEnergy = registry.getParameter<double>(params, prefix + "SigmaEnergy", 10.0);
            double thresholdEnergy = registry.getParameter<double>(params, prefix + "ThresholdEnergy", 0.0);
            double bSp = registry.getParameter<double>(params, prefix + "B_sp", -1.0);

            model->addIonParticle(sourcePower, 0.0, 90.0, 80.0, bSp,
                                 meanEnergy, sigmaEnergy, thresholdEnergy,
                                 89.0, 10.0, "ionFlux");

            VPSLOG_INFO(nullptr, "Added ion particle ", i , " with mean energy = ", meanEnergy, " eV");
        }

        double sign = (processType == 0) ? -1.0 : 1.0; // negative for etching
        switch(rateFunctionType) {
            case 0:
                model->setRateFunction([overallRate, sign](const std::vector<NumericType> &fluxes,
                                                           const viennaps::Material &material) {
                    return sign * overallRate * std::accumulate(fluxes.begin(), fluxes.end(), 0.0);
                });
                break;
            case 1:
                model->setRateFunction([overallRate, sign, numNeutrals](const std::vector<NumericType> &fluxes,
                                                                        const viennaps::Material &material) {
                    NumericType sum = 0;
        ...
        VPSLOG_DEBUG(nullptr, "Process type: ", (processType == 0 ? "Etching" : "Deposition"));
...
void registerSingleParticleProcessModel() {
    ...
    singleParticleProcess.description = "Single particle etching/deposition with diffuse reflections";
    singleParticleProcess.type = ModelType::PROCESS;

    ParameterMetadata rateParam;
    rateParam.name = "Rate";
    rateParam.displayName = "Process Rate";
    rateParam.documentation = "Etch/deposition rate (nm/s). Positive for deposition, negative for etching";
    ...
    stickingParam.documentation = "Probability of particle sticking to the surface (0-1)";
    stickingParam.type = ParameterType::DOUBLE;
    ...
    sourceParam.documentation = "Exponent for the angular distribution of the particle source";
    ...
    depositionMaterialParam.documentation = "Material to deposit when rate is positive";
    depositionMaterialParam.type = ParameterType::ENUM;
    depositionMaterialParam.defaultValue = static_cast<int>(viennaps::Material::SiO2);
                      ...
                      const ParameterMap& params)
    {
        ...
        double stickingProbability = registry.getParameter<double>(params, "StickingProbability", 1.0);
        double sourcePower = registry.getParameter<double>(params, "SourcePower", 1.0);
        ...
        if (rate > 0) {
            ...
            if (params.find("DepositionMaterial") != params.end()) {
                int matId = registry.getParameter<int>(params, "DepositionMaterial",
        ...
        viennaps::SmartPointer<viennaps::ProcessModelCPU<NumericType, Dim>> model;

        if (!maskMaterials.empty()) {
            model = viennaps::SmartPointer<viennaps::SingleParticleProcess<NumericType, Dim>>::New(
                rate, stickingProbability, sourcePower, maskMaterials
            );
        } else {
            model = viennaps::SmartPointer<viennaps::SingleParticleProcess<NumericType, Dim>>::New(
                rate, stickingProbability, sourcePower
            );
        }
...
void registerDirectionalProcessModel() {
    ...
    directionalProcess.description = "Directional etching/deposition with visibility calculations";
    ...
    visibilityParam.documentation = "Enable visibility calculations for shadowing effects";
                      ...
                      const ParameterMap& params)
    {
        ...
        double directionalVelocity = registry.getParameter<double>(params, "DirectionalVelocity", 5.0);
        double isotropicVelocity = registry.getParameter<double>(params, "IsotropicVelocity", 0.0);

        bool calculateVisibility = registry.getParameter<bool>(params, "CalculateVisibility", true);

        std::vector<viennaps::Material> maskMaterials;
        if (params.find("MaskMaterials") != params.end()) {
            const auto& matList = std::get<MaterialListValue>(
                params.at("MaskMaterials")
            );
        ...
        model = viennaps::SmartPointer<viennaps::DirectionalProcess<NumericType, Dim>>::New(
    ...
    vtkViennaPSModelRegistry::getInstance().registerProcessModel("DirectionalProcess", directionalProcess, factory);
}
...
void registerIsotropicProcessModel() {
                      ...
                      const ParameterMap& params)
    {
        ...
        double processTime = registry.getParameter<double>(params, "ProcessTime", 5.0);
        double processRate = registry.getParameter<double>(params, "ProcessRate", 1.0);
        ...
        if (maskMaterials.empty()) {
            model = viennaps::SmartPointer<viennaps::IsotropicProcess<NumericType, Dim>>::New(
    ...
    vtkViennaPSModelRegistry::getInstance().registerProcessModel("IsotropicProcess", isotropicProcess, factory);
}

void registerCustomSphereDistributionModel() {
    using namespace ViennaPSMeta;

    ModelMetadata meta;
    meta.className = "CustomSphereDistribution";
    meta.displayName = "Custom Sphere Distribution";
    meta.description = "Deposits or etches a set of concentric spherical shells at user-defined radii";
    meta.type = ModelType::PROCESS;

    ParameterMetadata numRadiiParam;
    numRadiiParam.name = "NumberOfRadii";
    numRadiiParam.displayName = "Number of Radii";
    numRadiiParam.documentation = "How many concentric sphere radii to use (1-5)";
    numRadiiParam.type = ParameterType::INTEGER;
    numRadiiParam.defaultValue = 2;
    numRadiiParam.minValue = 1;
    numRadiiParam.maxValue = 5;
    numRadiiParam.category = ParameterCategory::BASIC;
    numRadiiParam.required = true;
    meta.parameters.push_back(numRadiiParam);

    const std::array<double, 5> defaultRadii = {5.0, 10.0, 15.0, 20.0, 25.0};
    for (int i = 1; i <= 5; ++i) {
        ParameterMetadata rParam;
        rParam.name = "Radius" + std::to_string(i);
        rParam.displayName = "Radius " + std::to_string(i);
        rParam.documentation = "Radius of sphere shell " + std::to_string(i) + " (nm)";
        rParam.type = ParameterType::DOUBLE;
        rParam.defaultValue = defaultRadii[i - 1];
        rParam.minValue = 0.1;
        rParam.maxValue = 1000.0;
        rParam.category = ParameterCategory::BASIC;
        rParam.unit = "";
        rParam.stepSize = 0.1;
        rParam.required = (i == 1);
        meta.parameters.push_back(rParam);
    }
    ...
    processTimeParam.stepSize = 0.1;
    processTimeParam.required = true;
    meta.parameters.push_back(processTimeParam);
    ...
    meta.parameters.push_back(maskParam);
    ...
    auto factory = [](std::shared_ptr<void> psDomainVoid,
        ...
        int numRadii = registry.getParameter<int>(params, "NumberOfRadii", 2);
        double processTime = registry.getParameter<double>(params, "ProcessTime", 1.0);

        std::vector<NumericType> radii;
        for (int i = 1; i <= numRadii; ++i) {
            double r = registry.getParameter<double>(params, "Radius" + std::to_string(i), 5.0 * i);
            radii.push_back(static_cast<NumericType>(r));
        }
        ...
        VPSLOG_INFO(nullptr, "CustomSphereDistributionModel: ", ViennaPSMeta::ParameterMapToString(params));

        ViennaPSModels::withDomain(psDomainVoid, dimension, [&](auto psDomain, auto dimTag) {
            constexpr int Dim = decltype(dimTag)::value;

            auto model = viennaps::SmartPointer<viennaps::CustomSphereDistribution<NumericType, Dim>>::New(
                radii
            );
            for (const auto &mat : maskMaterials) {
                model->addMaskMaterial(mat);
    ...
    vtkViennaPSModelRegistry::getInstance().registerProcessModel(
        "CustomSphereDistribution", meta, factory
    );
}
...
void ViennaPSModels::initializeDistributionModels() {
    ...
    registerCustomSphereDistributionModel();
}
